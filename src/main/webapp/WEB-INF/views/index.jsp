        ...
        <ul class="category-list">
            ...
            <c:forEach items="${categories}" var="cat">
                <c:if test="${cat.parentId == 0}">
                    <li>
                        <a href="${pageContext.request.contextPath}/product/list?categoryId=${cat.id}">${cat.name}</a>
                        <ul class="sub-category">
                            <c:forEach items="${categories}" var="sub">
                                <c:if test="${sub.parentId == cat.id}">
                                    <li><a href="${pageContext.request.contextPath}/product/list?categoryId=${sub.id}">${sub.name}</a></li>
                                </c:if>
                            </c:forEach>
                        </ul>
                    </li>
