...
<div class="order-detail">
    ...
    <div class="form-actions" style="margin-top:20px;">
        <c:if test="${order.status == 'paid'}">
            <a href="${pageContext.request.contextPath}/admin/order/ship/${order.id}" class="btn btn-info" onclick="return confirm('确认发货？')">发货</a>
        </c:if>
        <c:if test="${order.status == 'shipped'}">
            <a href="${pageContext.request.contextPath}/admin/order/complete/${order.id}" class="btn btn-success" onclick="return confirm('确认完成？')">完成</a>
        </c:if>
        <a href="${pageContext.request.contextPath}/admin/order/list" class="btn btn-default">返回</a>
    </div>
</div>
